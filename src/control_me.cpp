...
public:
    MyoMotor() {
        ...
        initialize_srv = nh->advertiseService("/roboy/initialize", &MyoMotor::initializeControllers, this);
        control_me_sub = nh->subscribe("/roboy/control_me/setpoint",100, &MyoMotor::controlMe, this);
    }
            ...
            switch ((uint) req.controlmode[i++]) {
                ...
                default:
                    ROS_WARN("The requested controlMode is not available, "
                                     "choose [1]PositionController [2]VelocityController [3]ForceController");
                    break;
            }
        }
    }
    ...
    */
    bool setupMotor(unsigned int controlMode) {
        enum controllerOptions {
            position, velocity, effort = 2, force = 2
        };
        switch (controlMode) {
            case position:
